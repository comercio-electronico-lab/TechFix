  ...
  const steps = useMemo<TimelineStep[]>(() => {
    ...
    return stages.map((stage, idx) => {
      ...
      return {
        ...
        date: log ? new Date(log.created_at).toLocaleString('es-ES', {
          day: '2-digit',
          month: 'short',
          hour: '2-digit',
          minute: '2-digit',
            ...
            {warranty ? (
              ...
              <div className="bg-white dark:bg-slate-900 border border-outline-variant/60 dark:border-slate-800 p-6 rounded-2xl shadow-sm space-y-4">
                <div className="flex items-center gap-2 text-slate-400">
                  ...
                  <h4 className="font-bold text-xs uppercase tracking-wider text-on-surface dark:text-slate-350">Certificado de Garantía</h4>
                </div>
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-surface dark:bg-slate-950">
        <div className="text-center space-y-4">
          ...
          <p className="text-on-surface-variant dark:text-slate-400 text-sm">Verificando autenticación...</p>
        </div>
      </div>
    );
  }

...
export default function TecnicoDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Adaptar el formato de estados para compatibilidad visual si es necesario
      const adaptedRepairs = repairsData.map((r: any) => ({
        ...r,
        // Si el estado es 'pending', 'in_progress', 'completed' los mantenemos
        // O si viene en el formato original de la cola de reparación
  ...
  const handleUpdateStatus = async (jobId: string, newStatus: 'pending' | 'in_progress' | 'completed') => {
    if (!token) return;
    // Mapear de vuelta al estado esperado por actions.ts
  ...
  const handleAddPartToJob = async (jobId: string, productId: string) => {
    ...
    try {
      // 1. Vincular parte al ticket (aumenta precio final)
      await addPartToRepairAction(token, jobId, productId, 1);
      
      // 2. Decrementar el inventario del repuesto

...
export const FeaturedProductShowcase = async () => {
  // Fetch products dynamically from the catalog
  const products = await getProducts();
  const displayProducts = products.slice(0, 4);

  return (
    <section className="w-full pt-3 pb-8 px-3 md:px-4">
      <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-3">
        {displayProducts.map((product, index) => {
          ...
          return (
            ...
            >
              <div 
                className={`relative h-[450px] md:h-[500px] rounded-[28px] overflow-hidden flex flex-col justify-between items-center pt-12 pb-0 px-6 transition-all duration-500 hover:scale-[1.005] shadow-md ${
                  isDark 
              ...
              >
                ...
                <div className="text-center z-10 max-w-[85%] space-y-3">
                  <h3 className="font-sans text-3xl md:text-[40px] font-semibold tracking-tight leading-tight">
                    {product.name}
                  </h3>
                  <p className={`font-sans text-base md:text-[19px] font-normal leading-relaxed px-2 ${isDark ? 'text-slate-400' : 'text-slate-500 dark:text-slate-350'}`}>
                    {product.description}
                  </p>

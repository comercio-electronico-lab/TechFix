  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-12 py-8 px-4">
      ...
      <div className="w-full bg-slate-200 dark:bg-slate-800/40 rounded-full h-1.5 overflow-hidden">
        <div 
          className="h-full bg-gradient-to-r from-primary via-secondary to-sky-400 dark:from-primary dark:via-sky-400 dark:to-sky-300 transition-all duration-500 ease-out" 
          style={{ width: `${progress}%` }} 
        />
      </div>

      <div key={step} className="animate-fade-in-slide w-full">
        ...
        {step === 4 && selectedDeviceType && selectedBrand && selectedModel && (
          <div className="space-y-4">
            ...
            <button onClick={nextStep} className="w-full px-6 py-3.5 bg-primary dark:bg-sky-500 text-white rounded-xl font-bold hover:scale-[1.01] active:scale-[0.99] transition-all cursor-pointer text-xs tracking-wider uppercase shadow-md hover:shadow-lg">
              Continuar
            </button>
          </div>
        )}
